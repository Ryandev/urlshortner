...
[ -n "$ACCESS_TOKEN" ] || abort "Failed to get access token from response: $ACCESS_TOKEN"
...
BUDGET_NAMES=$(curl -X GET --header "Authorization: Bearer $ACCESS_TOKEN" "$REST_BUDGET_LIST_API" | jq -cr '.value[].name') || abort "Failed to get budget names: $BUDGET_NAMES"
...
for BUDGET_NAME in $BUDGET_NAMES; do
    if [ -n "$BUDGET_NAME" ]; then
        ...
        curl -X DELETE --header "Authorization: Bearer $ACCESS_TOKEN" "$REST_BUDGET_DELETE_API" || abort "Failed to delete budget $BUDGET_NAME" 1>/dev/null || abort "Failed to delete budget: $BUDGET_NAME"
    fi
done
